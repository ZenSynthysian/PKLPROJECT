...
function DataKadivConfiguration() {
    ...
    const fetchData = useCallback(async () => {
        try {
            const response = await axios.get(`${import.meta.env.VITE_SERVER_API}api/kadivall?page=${page}`, {
    ...
    }, [page]); // Empty dependency array to fetch data only on mount
    ...
    async function handleDelete(id) {
        try {
            ...
            if (result.isConfirmed) {
                await axios.delete(`${import.meta.env.VITE_SERVER_API}api/kadiv/${id}`, {
                    withCredentials: true,
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
                });
                ...
                setTable((prevTable) => prevTable.filter((item) => item.nomor_pjk !== id));
        ...
        <>
            <div className="flex flex-col justify-center items-center pt-32 overflow-auto pl-32 pr-32 w-screen">
                <div className="flex justify-center w-full p-6">
                    <Link
                        to={'/datakadivconfiguration/newdatakadiv'}
                ...
                <div className="border-2 border-blue-400 bg-white">
                    <table id="table-main">
                        <thead className="bg-indigo-950 text-gray-300">
                            ...
                            <th className="p-1 w-[12rem] border-r-2 border-blue-400 whitespace-nowrap text-lg">Divisi</th>
                            ...
                            {table.map((data, index) => {
                                return (
                                    <tr
                                        ...
                                        <td className="p-1 border-r-2 border-blue-400 whitespace-nowrap text-center ">{data?.divisi}</td>
                                        ...
                                        <td className="border-blue-400 whitespace-nowrap text-center flex gap-3">
                                            <Link
                                                to={`/datakadivconfiguration/editdatakadiv/${data?.id}`}
                ...
                <div className="flex">
                    ...
                    {totalPage.slice(startPage, endPage).map((data, index) => (
                        <Link
                            key={index + 1}
                            to={`/datakadivconfiguration/${data}`}>
                            <div
...
export default DataKadivConfiguration;
